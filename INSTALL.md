...
|---|---|---|---|
| Fireside | среда | 09:00 | раз в 2 недели |
| This or That | пятница | 09:00 | раз в 2 недели |
| Weekly Theme | понедельник | 09:00 | раз в 2 недели (та же неделя, что This or That) |
| Spotlight | среда | 09:00 | раз в 2 недели (та же неделя, что Weekly Theme) |
| Friday Check-in | пятница | 15:00 | каждую неделю |
| What I'm Working On | 1-е число месяца | 09:00 | раз в месяц |

Weekly Theme/This or That/Spotlight и Fireside объединены в две группы, которые чередуются по неделям (ISO week number, чётность): на одной неделе выходят только Fireside (ср), на следующей — Weekly Theme (пн) + This or That (пт) + Spotlight (ср, если включён). Workflow всё равно запускается каждый понедельник/среду/пятницу, но скрипт проверяет `isGroupWeek()` в `src/utils.js` и сам решает, постить в этот раз или нет — это не требует отдельного state-файла и не собьётся при пропущенном запуске.
